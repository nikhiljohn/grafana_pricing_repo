...
Afterwards set the default branch — either **Settings → Repository → Branch
defaults → `main`**, or let `configure-gitlab.sh` do it (§3).
...
## 3. Project settings and CI/CD variables

All of this is REST API, so it does not need the UI:

```bash
# On the VPN. Needs a token with the `api` scope — NOT the write_repository
# token the migration used; those are separate scopes.
GITLAB_TOKEN=glpat-… bash deploy/gcp/configure-gitlab.sh
```

It sets the default branch to `main`, turns Auto DevOps off, and upserts the
variables below — idempotent, so re-running is safe. `DRY_RUN=1` shows the
changes without writing.

### Protected variables and protected branches are one decision

The table says *Protect: yes* for `GCP_SA_KEY`, and that is the point of it:
unprotected, the service account JSON is readable by a pipeline on any branch
anyone can push. But a **protected variable is invisible to pipelines on an
unprotected branch** — so protected variables plus an unprotected `main` means
`deploy:production` runs with an empty `GCP_SA_KEY` and dies at `gcloud auth`,
looking for all the world like a credentials problem.

So the script protects `main` (Maintainers may push, force push off) before
creating protected variables. `PROTECT_MAIN=no` skips that and creates the
variables unprotected instead — acceptable for a private repo with a small
team, not once outside contributors can push.

Note this interacts with §2: if you protect `main`, a later `FORCE_MAIN=yes`
will be rejected until you allow force push again.
...
`STAGING_DOMAIN`. The script sets these only if you pass them in the
environment, since there is no staging VM today.

`GCP_SA_KEY` is the one the script may not be able to fill: the JSON is
generated by `setup-searce-gcp.sh` into whatever directory it ran in — usually
Cloud Shell, not a laptop. If it isn't found the other five are still set, and
the script prints the `gcloud iam service-accounts keys create` command to mint
a fresh key (a service account can hold several, so this doesn't invalidate
anything).
