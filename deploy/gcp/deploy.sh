...
bold "── Google OAuth (Sign in with Google) ─────────────────────────────"
echo "  Create credentials at: console.cloud.google.com → APIs & Services → Credentials"
echo "  Authorized redirect URI to register: https://${DOMAIN}/api/auth/oauth/google/callback"
echo "  Leave blank to skip — users will sign in with email/password only."
